			...
			<?php
				...
				if (count($fotoEspacos > 0)) {
					for ($i=0; $i < count($fotoEspacos); $i++) { 
						...
						echo $this->Form->postLink('<span class="btn btn-danger" role="button">Excluir</span>', array('action' => 'delete', $fotoEspacos[$i]['FotoEspaco']['id'], $espaco['EspacoEvento']['id']), array('escape' => false), __('Tem certeza que deseja escluir?'));
						echo '</div>';
						echo '</div>';
						echo '</div>';
					}
