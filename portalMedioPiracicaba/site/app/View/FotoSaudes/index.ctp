			...
			<?php
				...
				if (count($fotoSaudes > 0)) {
					for ($i=0; $i < count($fotoSaudes); $i++) { 
						...
						echo $this->Form->postLink('<span class="btn btn-danger" role="button">Excluir</span>', array('action' => 'delete', $fotoSaudes[$i]['FotoSaude']['id'], $saude['OrgaoSaude']['id']), array('escape' => false), __('Tem certeza que deseja escluir?'));
						echo '</div>';
						echo '</div>';
						echo '</div>';
					}
