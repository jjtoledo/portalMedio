			...
			<?php //debug($fotos) 
				...
				if (count($fotoDistritos > 0)) {
					for ($i=0; $i < count($fotoDistritos); $i++) { 
						...
						echo $this->Form->postLink('<span class="btn btn-danger" role="button">Excluir</span>', array('action' => 'delete', $fotoDistritos[$i]['FotoDistrito']['id'], $distrito['Distrito']['id']), array('escape' => false), __('Tem certeza que deseja escluir?'));
						echo '</div>';
						echo '</div>';
						echo '</div>';
					}
